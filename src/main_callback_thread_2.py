...
#        self.__NoiseStreamMaker = None
    ...
    @NoiseColor.setter
    def NoiseColor(self, v):
        self.__NoiseStreamMaker.Color = v
#        self.stop()
#        self.start()
...
#        noise_color = 'pink'
        #ノイズデータのコールバック実装クラス
#        self.__NoiseStreamMaker = NoiseStreamMaker(hz=rate, second=second, noise_color=self.NoiseColor)
        #再生準備する
        self.__pyaudio = pyaudio.PyAudio()
        self.__stream = self.__pyaudio.open(format=format,
                        channels=channels,
...
#        self.thread = None#効果なし

#    def set_color(self, color):
#        self.set_noise_color_event.set()
#        noise_color
...
#    th.start()
    ...
    while True:
        print('input noise color: [w]hite, [p]ink, brow[n], [b]lue, [v]iolet:')
